...
ENABLE_GOOGLE_SHEETS = True  # Fitur Aktif

# KREDENSIAL YANG ANDA BERIKAN (DITANAM LANGSUNG)
...
def get_gspread_client():
    ...
    try:
        # Load langsung dari String Variabel di atas
...
async def process_kpro_logic(raw_df):
    ...
    if not client: return False, "⚠️ Gagal koneksi Google Sheet (Credentials Invalid).", {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    text = (
        ...
        "2. Bot akan **Mengupdate Google Sheet** dan memberikan rincian teks.\n"
    )
